[← Home](https://imageslr.github.io/)

# 📔【操作系统】I/O 多路复用，select / poll / epoll 详解

Feb 27, 2020 • 2542 View

- 从阻塞 I/O 到 I/O 多路复用
  - [为什么 I/O 多路复用内部需要使用非阻塞 I/O](https://imageslr.github.io/2020/02/27/select-poll-epoll.html#为什么-io-多路复用内部需要使用非阻塞-io)
- select
  - [函数签名与参数](https://imageslr.github.io/2020/02/27/select-poll-epoll.html#函数签名与参数)
  - [什么是文件描述符 fd](https://imageslr.github.io/2020/02/27/select-poll-epoll.html#什么是文件描述符-fd)
  - [socket 与 fd 的关系](https://imageslr.github.io/2020/02/27/select-poll-epoll.html#socket-与-fd-的关系)
  - [fd_set 文件描述符集合](https://imageslr.github.io/2020/02/27/select-poll-epoll.html#fd_set-文件描述符集合)
  - [select 使用示例](https://imageslr.github.io/2020/02/27/select-poll-epoll.html#selectdemo)
  - [select 的缺点](https://imageslr.github.io/2020/02/27/select-poll-epoll.html#select-的缺点)
- [poll](https://imageslr.github.io/2020/02/27/select-poll-epoll.html#poll)
- epoll
  - [epoll_create](https://imageslr.github.io/2020/02/27/select-poll-epoll.html#epoll_create)
  - [epoll_ctl](https://imageslr.github.io/2020/02/27/select-poll-epoll.html#epoll_ctl)
  - [epoll_wait](https://imageslr.github.io/2020/02/27/select-poll-epoll.html#epoll_wait)
  - [epoll 的优点](https://imageslr.github.io/2020/02/27/select-poll-epoll.html#epoll-的优点)
  - [水平触发、边缘触发](https://imageslr.github.io/2020/02/27/select-poll-epoll.html#水平触发边缘触发)
  - [为什么边缘触发必须使用非阻塞 I/O？](https://imageslr.github.io/2020/02/27/select-poll-epoll.html#whynonblock)
- [三者对比](https://imageslr.github.io/2020/02/27/select-poll-epoll.html#三者对比)
- [适用场景](https://imageslr.github.io/2020/02/27/select-poll-epoll.html#适用场景)
- [Redis 的线程模型](https://imageslr.github.io/2020/02/27/select-poll-epoll.html#redis-的线程模型)
- [🗂 技术面试题汇总](https://imageslr.github.io/2020/07/08/tech-interview.html)
- [参考资料](https://imageslr.github.io/2020/02/27/select-poll-epoll.html#参考资料)

> 更多面试题总结请看：[🗂【面试题】技术面试题汇总](https://imageslr.github.io/2020/07/08/tech-interview.html)
...
![15732186159520](https://imageslr.github.io/media/15732186159520.jpg)

### select 的缺点

1. 性能开销大
   1. 调用 `select` 时会陷入**内核**，这时需要将参数中的 `fd_set` 从用户空间拷贝到内核空间
   2. 内核需要遍历传递进来的所有 `fd_set` 的每一位，不管它们是否就绪
2. 同时能够监听的文件描述符数量太少。受限于 `sizeof(fd_set)` 的大小，在编译内核时就确定了且无法更改。一般是 1024，不同的操作系统不相同

## poll

poll 和 select 几乎**没有区别**。poll 采用链表的方式存储文件描述符，没有最大存储数量的限制。
...
- `select` 可以使用阻塞 I/O
  ...
  获取到所有可读的文件描述符后，遍历每个文件描述符，`read` 一次, 数据（见上文select 示例）
        ...
        for (int i = 0; i < FD_SETSIZE; i++) {
            if (FD_ISSET(i, &read_fds)) {
                ...
                for (;;) {
                    ...
                    /* read has to be non-blocking for edge-triggered polling, eacuase the last read will always empty and it will block*/
